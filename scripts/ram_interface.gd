...
func ekrani_tazele():
	...
	# 2. RAM SEVİYE KONTROLLERİ
	if Global.ram_level == 2:
		%Btn1.visible = false       
		%LblMevcut1.visible = true  # "BELLEK FREKANSI YÜKSELDİ"
		
	elif Global.ram_level == 3:
		%Btn1.visible = false
		%Btn2.visible = false
		%LblMevcut2.visible = true  # "ÇİFT KANAL AKTİF"
		
	elif Global.ram_level >= 4:
		%Btn1.visible = false
		%Btn2.visible = false
		%Btn3.visible = false
...
func _on_btn_1_pressed() -> void:
	if Global.ram_level == 1 and Global.toplanan_bocek >= bedel_1:
		Global.toplanan_bocek -= bedel_1
		Global.ram_level += 1
		ekrani_tazele()

func _on_btn_2_pressed() -> void:
	if Global.ram_level == 2 and Global.toplanan_bocek >= bedel_2:
		Global.toplanan_bocek -= bedel_2
		Global.ram_level += 1
		ekrani_tazele()

func _on_btn_3_pressed() -> void:
	if Global.ram_level == 3 and Global.toplanan_bocek >= bedel_3:
